...
LogicSystem::LogicSystem()
{
	...
	RegPost("/user_login", [](std::shared_ptr<HttpConnection> connection) {
		...
		int userID;
		//查询数据库判断用户名和密码是否匹配
		bool pwd_valid = MysqlMgr::GetInstance()->CheckPwd(name, pwd, userID);
		if (!pwd_valid) {
		...
		auto reply = StatusGrpcClient::GetInstance()->GetChatServer(userID);
		if (reply.error()) {
		...
		std::cout << "succeed to load userinfo uid is " << userID << std::endl;
		root["error"] = 0;
		root["user"] = name;
		root["uid"] = userID;
